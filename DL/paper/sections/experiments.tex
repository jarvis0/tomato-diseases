...
\begin{table}
	...
	\begin{center}
		\begin{tabular}{|c|c|c|}
			...
			& Original & 
					\begin{tabular}{@{}c@{}}
					$Average Accuracy = \textbf{0.932301}$ \\
					$Precision_{M} = \textbf{0.925955}$ \\
					$Recall_{M} = \textbf{0.913326}$ \\
					$F_{1} score_{M} = \textbf{0.916384}$ \\
					\end{tabular} \\
			\cline{2-3}
			& Segmented &
				\begin{tabular}{@{}c@{}}
				$Average Accuracy = \textbf{0.949766}$ \\
				$Precision_{M} = \textbf{0.942972}$ \\
				$Recall_{M} = \textbf{0.928461}$ \\
				$F_{1} score_{M} = \textbf{0.934499}$ \\
				\end{tabular} \\  				
			\hline
		\end{tabular}
	\end{center}
	\caption{\textit{Performance of multi-class classification with difference training and test sets. We use a NaN value for the class that are never predicted to avoid division by zero in precision, recall and F1 score.}}
...
\begin{figure}[t]
	\begin{center}
		\fbox{\includegraphics[scale=0.55]{./images/conf_matrix_norm}}
	\end{center}
