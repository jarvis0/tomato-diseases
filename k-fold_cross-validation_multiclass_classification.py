    ...
    for epoch in epoch_progress:  # loop over the dataset multiple epochs
        ...
        for phase in ['train', 'val']:
            ...
            for inputs, labels in dataloaders[phase]:
                ...
                running_corrects += torch.sum(preds == labels.data)#torch.sum(preds.view(-1) == labels.data.float())
...
"""# Statistical approach - useless
total_epoch_performace = [0] * num_epochs
for epoch in range(num_epochs):
    for k in range (num_folds):
        total_epoch_performace[epoch] += folds_performances[k][epoch]

# Mitchell approach - useless
total_max_performance = [0] * num_folds
for k in folds_performances:
    total_max_performance[k] = folds_performances[k].index(max(folds_performances[k]))
